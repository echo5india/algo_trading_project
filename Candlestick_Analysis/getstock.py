...
def getMinuteFXPrices(pair):
    print(pair[:3],pair[3:])
    url = 'https://www.alphavantage.co/query?function=FX_INTRADAY&from_symbol={}&to_symbol={}&interval=1min&apikey={}&datatype=csv&outputsize=full'.format(pair[:3],pair[3:],apikey)
    s = requests.get(url).content
    symbol_df = pd.read_csv(io.StringIO(s.decode('utf-8')))
    symbol_df = symbol_df.sort_values('timestamp', ascending=True)
    converted_timestamp = pd.to_datetime(symbol_df['timestamp'], infer_datetime_format=True)
